...
val javacInternalPackages = listOf(
  "file",
  "parser",
  "tree",
  "util",
...
// `?:` tests only for null, so the empty string is rejected above rather than defaulted here.
val testJavaVersion =
  JavaLanguageVersion.of(
    (testJavaVersionProperty ?: JavaVersion.current().majorVersion).toString(),
  )
...
spotless {
  ...
  kotlinGradle {
    ...
    // There is no .editorconfig file, so state ktlint's settings here.  The 100-column limit
    // matches what googleJavaFormat above uses for Java.
...
//
// This is a method of an object rather than a top-level function, because Kotlin compiles a
// top-level function of a build script into a method of the script class, so calling it from a task
// action would capture the script -- and thus the "Project" object -- in the action.
object JavadocFonts {
...
tasks.withType<Javadoc>().configureEach {
  ...
  options.quiet()
  doLast {
    JavadocFonts.removeDejaVuFontImport(destinationDir!!)
  }
}
...
val javadocPrivate = tasks.register<Javadoc>("javadocPrivate") {
  group = "documentation"
...
tasks.check {
  dependsOn(tasks.javadoc, javadocPrivate)
}

val javadocWebUpload = tasks.register<Javadoc>("javadocWebUpload") {
...
// Set permissions
val javadocWebChgrp = tasks.register<Exec>("javadocWebChgrp") {
...
val javadocWebChmod = tasks.register<Exec>("javadocWebChmod") {
...
dependencies {
  requireJavadocConfiguration.name(libs.require.javadoc)
}
...
val requireJavadoc = tasks.register<JavaExec>("requireJavadoc") {
  group = "documentation"
